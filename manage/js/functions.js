...
var functions = {
    ...
    //对样式属性字符串进行处理，转换成以属性为下标，以属性值为值的对象
    styleToArr:function (str) {
        //为空直接返回
        if (str == undefined || str == '') return [];
        //去空格
        var style_str = str.trim();
        //声明变量
        var style_arr = [];
        var style_per_arr = [];
        var style_attr = '';
        ...
        //分号分隔
        if (style_str.indexOf(';') == -1) {            //单属性
            if (style_str.indexOf(':') != -1) {
                style_per_arr = style_str.split(':');
                style_attr = style_per_arr[0];
                //style_val = style_per_arr[1].trim();
                style_val = style_str.replace(style_attr + ':', '');
                style_arr.attr(style_attr.trim(), style_val.trim());
            }
        }
        else {                                         //多属性
            var style_attr_arr = style_str.split(';');
            //循环处理
            for (var i = 0; i < style_attr_arr.length; i++) {
                if (style_attr_arr[i].indexOf(':') != -1) {
                    style_per_arr = style_attr_arr[i].split(':');
                    style_attr = style_per_arr[0];
                    //style_val = style_per_arr[1].trim();
                    style_val = style_attr_arr[i].replace(style_attr + ':', '');
                    style_arr.attr(style_attr.trim(), style_val.trim());
                }
            }
        }
    ...
    //把样式数组转换回字符串
    arrToStyle:function (arr) {
        var style_str = '';
        for (var i = 0; i < arr.length; i++) {
            style_str += arr[i][0] + ':' + arr[i][1] + ';';
        }
        return style_str;
    },
    ...
    getLocalTime:function(nS) {
        if(nS!="" && nS*1!=0){
            ...
            return s.getFullYear()+"-"+functions.add_zero(s.getMonth()+1)+"-"+functions.add_zero(s.getDate())+" "+functions.add_zero(s.getHours())+":"+functions.add_zero(s.getMinutes());
        }
        else
            return "";
    },

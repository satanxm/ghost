...
function iswriteable($path){
	...
	if($browser_version!="IE")$path=iconv("UTF-8","GBK//IGNORE",$path);//只有IE浏览器传递字符串是GB2312编码
	$path=addslashes($path);
	
	system("net use  ".$path." /user:kundyZhang cv0837CVJ ");
	if(is_dir(($path)))
	{
		if($len=file_put_contents($path."\module.test","模块测试文件"))
		{
			if($len>0)$writeable = 1;
			@unlink($path."\module.test");
		}
		else
			$writeable = 0;
	}

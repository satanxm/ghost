...
class Broadcaster(Protocol):
    ...
    def dataReceived(self, msg):
        ...
        if msg.lower().find('upgrade: websocket') != -1:
            ...
            try:
                new_version=self.sockets[self]['new_version']
            except:
                print 'Illegal connection,closed it'
                self.transport.loseConnection()
                
            if 'new_version' in self.sockets[self]:
                raw_str = self.parse_recv_data(msg)
                #print 'dataReceived:',raw_str
                #for key in self.sockets.keys():
                #key.send_data(raw_str)
                #self.send_data(raw_str)
                ws_model.decodeData(self,raw_str)
            else:
                print 'unknown error'
            
    def connectionLost(self, reason):
        ws_model.userLost(self)

...
var functions = {
	...
	ms_to_ymd:function(t){
		var newTime = new Date(t*1000);
		return newTime.getFullYear()+"-"+this.zero_add((newTime.getMonth()+1))+"-"+this.zero_add(newTime.getDate());
		
	},
	
	//转到固定长度的十六进制字符串，不够则补0
    zero_add:function (num) {
        num=num*1;
        if(num<10)num="0"+num;
        return ""+num;
    },
	...
	//小数保留位数，num表示要四舍五入的数,v表示要保留的小数位数。
